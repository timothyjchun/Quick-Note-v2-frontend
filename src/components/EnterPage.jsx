...
const EnterPage = ({ setHasAccess }) => {
  ...
  const enterPageForm = useRef(null);

  const enterPage = async (e) => {
    e.preventDefault();
    axios
      .post(
        `${BASE_URL}api/authenticate_user/`,
        {
  ...
  return (
    <>
      <div
        className="blur"
        style={{
          visibility: isEnteringPage ? "visible" : "hidden",
          backgroundColor: whichMemo.colorCode + "10",
        }}
      >
        <div className="blurPageScaffold">
          <button
            ...
            onClick={() => {
              setIsEnteringPage(false),
                setWhichMemo({ ...whichMemo, currentID: 0 });
            }}
            type="button"
          >
            <svg
          ...
          <form onSubmit={enterPage} ref={enterPageForm}>
            <div className="createPageMainContainer">
              <div className="createPageContainer">
                <div className="createPageDesc">
                  <p id="title">Page Title</p>
                </div>
                <div className="createPageInput divst">{whichMemo.title}</div>
              </div>
              <div className="createPageContainer">
                <div className="createPageDesc">
                  <p id="title">ID</p>
                </div>
                <div className="createPageInput divst">{whichMemo.id}</div>
              </div>
              <div className="createPageContainer">
                <div className="createPageDesc">
